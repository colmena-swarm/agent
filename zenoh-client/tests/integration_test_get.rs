...
#[cfg(test)]
mod tests {
    use zenoh_client::{get_published_service_definitions, zenoh_session};
    
    #[tokio::test(flavor = "multi_thread", worker_threads = 5)]
    async fn test_get_forwards_service_definition_correctly() {
        ...
        std::env::set_var("ZENOH_SESSION_START_DELAY", "0");
        let handle = tokio::spawn(async { get_published_service_definitions(downstream_services).await });
