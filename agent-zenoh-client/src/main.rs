...
fn compare_values_to_threshold(comparison_str: &str, threshold: f32, values: &Vec<f32>) -> bool{
    ...
    println!("Comparing received values to threshold: {}, comparison: {}", threshold, comparison_str);
    let mut values_iter = values.into_iter();
    return values_iter.find(comparison).is_none();
}

fn parse_comparison(comparison: &str, threshold: f32) -> Box<dyn FnMut(&&f32) -> bool> {
    match comparison {
        "<"     => Box::new(move | x| **x >= threshold),
        ">"     => Box::new(move | x| **x <= threshold),
        ">="     => Box::new(move | x| **x < threshold),
        "<="     => Box::new(move | x| **x > threshold),
...
async fn service_definition_subscription() {
    ...
    let _ = tokio::task::spawn(async move {
        while let Ok(sample) = subscriber.recv_async().await {
            ...
            let _ = client.post("http://127.0.0.1:50551")
