...
func matchstop(device device.Info, roles []Role, kpiMatcher KpiMatcher) []Role {
	...
	for _, role := range roles {
		if (qosMet(role, kpiMatcher) && !hasWorkload(role, kpiMatcher)) {
...
func hasWorkload(role Role, kpiMatcher KpiMatcher) bool {
	kpi := KPI{
		Key:           	"num_executions_" + device.Hostname() + "_" + role.Id,
		Threshold:     	1,
		ThresholdType: 	"GREATER_THAN_OR_EQUAL_TO",
		FromUnit:      	5,
		FromType:	  	"m",
		Comparison:		">",
	}
